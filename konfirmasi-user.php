...
<html lang="en">
  ...
  ?>
    <div class="container">
      <?php
      $result2 = mysqli_query($koneksi,"SELECT * FROM user WHERE status = 'Pending'");
       ?>
          <h1 class="flow-text center">Pengajuan User</h1>
              <table class="table highlight responsive-table">
                ...
                <tbody>
                  <tr>
                    ...
                    <td><?= $user["status"]; ?></td>
                    <td><?= $user["level"]; ?></td>
                    <td> <a href="konfirmasi.php?id=<?= $user['id'];?>" class="btn cyan accent-2 black-text waves-effect">Terima</a> |
